...
消息报头 可以让服务器包含关于响应请求的额外信息。比如，如果需要身份验证，服务器使用消息报头来表明身份验证类型。最经常遇到消息报头，是Content-Type显示请求成功与失败，用来帮助浏览器解析body。Headers以一个空白行结束，无论是否有消息报头都要有空白行。
...
响应的正文后有一个空行可以是任意数据。在一个典型的web请求中，通常是显示的HTML文档。如果遇到错误，信息body通常包含服务器生成的错误信息（通常是HTML，但是有时不是）用来解释错误。
